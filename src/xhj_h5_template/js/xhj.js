...
var XHJ = {
    //微信跳转地址
    wechatJumpUrl:"//api-a.xhangjia.com/wechat/scopes?redirect_uri=",

    //微信获取用户接口地址
    wechatUserApiUrl:"//api-a.xhangjia.com/api/wechat/userinfo",

    //微信SDK接口地址
    wechatSdkApiUrl:"//api-a.xhangjia.com/api/wechat/jssdk",

    //CDN地址
    cdnUrl:"//img.xhangjia.com",
    ...
    pcQrcode:function(){
        ...
        $("body").append('<div id="pcQrcode" style="position:fixed;z-index:9999999999999;background:#000;width:100%;height:100%;left:0;top:0;"></div>');
        var qrcode = new QRCode(document.getElementById('pcQrcode'),localhost);
        $("#pcQrcode img").css({
            "position":"absolute",
            "left":"50%",
            "top":"45%",
        ...
        $("#pcQrcode").append('<div style="color:#fff;font-size:20px;position:absolute; width:500px; left:50%; margin-left:-250px; text-align:center; top:50%; margin-top:200px;">为了更好的体验，请使用手机扫描二维码浏览</div>')
    },
    ...
    init:function(){
        ...
        if(XHJ._isWeixin()){
            this.wechat.sdkReady();

            //微信分享
            // XHJ.wechat.share('分享标题','分享描述');
            //获取微信用户信息
            // XHJ.wechat.getUserInfo(function(userInfo){
     ...
     * @param {[type]} minNum [最小数字]
     * @param {[type]} maxNum [最大数字]
     */
    ...
    wechat:{
        ...
        */
        share:function(shareTitle,shareDesc,link){
            ...
            wx.ready(function () {
                ...
                wx.onMenuShareAppMessage({
                    ...
                    desc: shareDesc,
                    link: link,
                    imgUrl: 'https://img.xhangjia.com/images/xhj_logo.png',   //分享的缩略图
                    success: function () { 
        ...
        getUserInfo(callback){
            if(XHJ._isWeixin()){
                ...
                if(XHJ.getQueryString("code")){
                    var code = XHJ.getQueryString("code");
                    $.ajax({
                        type:'GET',
                        url:XHJ.wechatUserApiUrl,
                        data:{
                            code:code
                        },
                        headers:{
                    ...
                    var localhost = encodeURIComponent(window.location.href)
                    window.location.href = XHJ.wechatJumpUrl+localhost;
                }
            }else{
